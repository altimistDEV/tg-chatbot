...
export class UserService {
  ...
  private readonly TEST_WALLET = '0x0ed637de4b9ccebe6d69991661a2d14f07f569d0';

  /**
   * Link a wallet address to a user
   */
  async linkUserWallet(userId: number, walletAddress: string): Promise<void> {
    try {
   ...
   */
  async getUserWallet(userId: number): Promise<string | null> {
   ...
   */
  async hasLinkedWallet(userId: number): Promise<boolean> {
   ...
   */
  async unlinkUserWallet(userId: number): Promise<void> {
    const user = this.users.get(userId);
    if (user) {
