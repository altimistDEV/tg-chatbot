 ...
 */
export function convertMarkdownToHTML(message: string): string {
  return message
    // Headers
    .replace(/^### (.*$)/gm, '<h3>$1</h3>')
    .replace(/^## (.*$)/gm, '<h2>$1</h2>')  
 ...
 */
export function convertMarkdownToWhatsApp(message: string): string {
 ...
 */
export function convertMarkdownToDiscord(message: string): string {
  return message
 ...
 */
export function escapeForFormat(text: string, format: MessageFormat): string {
  switch (format) {
    case 'html':
      return text
        .replace(/&/g, '&amp;')
 ...
 */
export function getMaxLength(format: MessageFormat): number {
  const maxLengths: Record<MessageFormat, number> = {
    whatsapp: 65536,
    discord: 2000,
    html: 4096,
    markdown: 4096,
