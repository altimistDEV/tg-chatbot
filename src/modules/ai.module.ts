...
export class AIModule extends BaseModule {
   ...
   */
  private async performWebSearchIfNeeded(text: string): Promise<WebSearchResult[] | null> {
    ...
    try {
      const results: SerpApiResponse = await getJson({
  ...
  private async buildSystemPrompt(
    ...
    const isServiceQuery =
      text.toLowerCase().includes('altimist') ||
   ...
   */
  private prepareMessages(history: MessageContext['history'], currentText: string): AnthropicMessage[] {
   ...
   */
  private async getAIResponse(messages: AnthropicMessage[], systemPrompt: string): Promise<string> {
    const response: AnthropicResponse = await this.anthropic.messages.create({
